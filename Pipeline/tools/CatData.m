...
for taski = 1:length(tasks)
    ...
    for i = 1:length(subs)
        ...
        for e = 1:3
            ...
            func_Realigned(:,:,:,:,e) = double(load_nii(dir_echo_Realigned).img);
        end
        ...
        if replace == 1
            dir_data_replace = fullfile(Parameter.dir_model_source, tasks{taski}, modality{1},'t2s');
            dir_sub_replace = [dir_data_replace 'sub-' subs{i} '_task-' tasks{taski} '.mat'];
            ...
            replace_name = [dir_MEICA '\tedana_output\t2s.nii'];
            save_nii(nii_replace,replace_name);
            gzip(replace_name,[dir_MEICA '\tedana_output']);
        end
    end
end
