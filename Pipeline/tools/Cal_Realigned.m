...
for taski = 1:length(tasks)
    ...
    for i = 1:length(subs)
        ...
        for e = 1:3
            ...
            nii_file = load_nii(dir_echo_Realigned);
            func_Realigned(:,:,:,:,e) = nii_file.img;
            if e == 2
                save_hdr = nii_file.hdr;
            end
        end
...
%             
            if exist([dir_sub modality{j}])
                movefile([dir_sub modality{j}],dir_mod);
            end       
%             
            save_name_mod = fullfile(dir_mod, 'func', '4D_Realigned.nii');
%             
            if exist(save_name_mod)
                disp(['pass' save_name_mod]);
                continue;
            end     
%             
            if ~exist(dir_mod)
