...
for taski = 1:length(tasks)
    ...
    for i = 1:length(subs)
        ...
        dir_funcraw2 = fullfile(dir_func2, ['sub-' subs{i} '_task-' tasks{taski} '_echo-2_bold.nii']);
        dir_func3 = fullfile(dir_data, ['sub-' subs{i}], 'echo3', 'func');
        dir_funcraw3 = fullfile(dir_func3, ['sub-' subs{i} '_task-' tasks{taski} '_echo-3_bold.nii']);
